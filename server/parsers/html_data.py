...
class NextbikeHistoryParser(HTMLParser):
    ...
    def swipe_station_name_to_id(self, account_history_string):
        for station_id in self.station_id_name_dict:
            account_history_string = account_history_string.replace(
                self.station_id_name_dict[station_id].encode('ascii', 'ignore'), station_id.encode('ascii', 'ignore'))
    ...
    def station_str_to_dic(self, station_str):
        ...
        m = re.search(
            '(\d\d\.\d\d\.\d\d)\s{1,3}(\d\d:\d\d)\s{1,3}([a-z|A-Z]{1,10})\s{1,3}(\d{1,6})\s{1,3}\w{1,3}\s{1,3}(\d\d:\d\d:\d\d)\s{1,3}\((\d{1,6})\s{1,3}\-\s{1,3}(\d{1,6})\)',
            station_str)
        if m and m.group(0):
    ...
    def handle_starttag(self, tag, attrs):
        if tag == UNORDERED_LIST_ELEMENT and (
                ACCOUNT_HISTORY_LIST_TAG_ATTRIBUTE_KEY, ACCOUNT_HISTORY_LIST_TAG_ATTRIBUTE_VALUE) in attrs:
            self.in_account_history_list_tag = True
        elif self.in_account_history_list_tag and tag == ORDERED_LIST_ELEMENT and (
                ACCOUNT_HISTORY_TAG_ATTRIBUTE_KEY, ACCOUNT_HISTORY_TAG_ATTRIBUTE_VALUE) not in attrs:
            self.in_account_history_tag = True
        elif self.in_account_history_tag and tag == SPAN_ELEMENT_ELEMENT and (
                ACCOUNT_HISTORY_PRICE_TAG_ATTRIBUTE_KEY, ACCOUNT_HISTORY_PRICE_TAG_ATTRIBUTE_VALUE) in attrs:
